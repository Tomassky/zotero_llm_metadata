    ...
    try:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                logger.debug("temp file_path=%s", file_path)
...
# Build, cluster, and analyze a NetworkX graph from Zotero items

_GRAPH_OUTPUT_DIR = Path.home() / ".config" / "zotero-mcp" / "graph"


@mcp.tool(
    name="zotero_build_graph",
    description="Build a knowledge graph from your Zotero library using collection and tag relationships, then run Leiden community detection."
)
def build_graph(
    max_degree: int = 20,
    tag_max_fraction: float = 0.30,
    ...
    *,
    ...
    """
    Build a NetworkX knowledge graph from all Zotero items.

    Edges come from shared collections (strong) and shared tags (medium).
    Runs Leiden/Louvain community detection and exports results.

    Args:
        max_degree: Max edges per node (anti-hairball control). Default 20.
        tag_max_fraction: Max fraction of items a tag can cover. Default 0.30.
        collection_weight: Edge weight for same_collection edges. Default 2.0.
        tag_weight: Edge weight for shared_tag edges. Default 1.0.
    """
    try:
        from zotero_mcp.graph_builder import build_graph as _build_graph, TagFilter, graph_stats
        from zotero_mcp.graph_cluster import cluster, score_all, modularity, label_communities, validate_communities
        from zotero_mcp.graph_analyze import god_nodes, surprising_connections, suggest_questions
        from zotero_mcp.graph_export import to_json, to_html, generate_report

        ctx.info("Building knowledge graph from Zotero library...")
        ...
        if not all_items:
            return "No items found in Zotero library."

        ctx.info(f"Found {len(all_items)} items. Building graph...")

        # Build graph
        tag_filter = TagFilter(max_fraction=tag_max_fraction)
        G = _build_graph(
            all_items,
        ...
        if G.number_of_nodes() == 0:
            return "Graph is empty — no items with valid connections."
        ...
        json_path = str(_GRAPH_OUTPUT_DIR / "graph.json")
        html_path = str(_GRAPH_OUTPUT_DIR / "graph.html")
        report_path = str(_GRAPH_OUTPUT_DIR / "GRAPH_REPORT.md")

        to_json(G, communities, community_labels, cohesion, json_path)
        try:
            to_html(G, communities, community_labels, html_path, cohesion_scores=cohesion)
        except ValueError as e:
            html_path = None  # Too large for viz

        report_text = generate_report(
            G, communities, community_labels, cohesion, validation,
            god_list, surprise_list, question_list,
        )
        Path(report_path).write_text(report_text, encoding="utf-8")

        # Format output
        output = ["# Zotero Knowledge Graph Built", ""]
        output.append(f"**Items:** {stats['nodes']} · **Connections:** {stats['edges']} · **Avg degree:** {stats['avg_degree']}")
        output.append(f"**Communities:** {len(communities)} · **Modularity:** {validation['modularity']}")
        ...
        for cid, members in communities.items():
            ...
            output.append(f"- **{label}** ({len(members)} items, cohesion: {coh})")

        output.append("")
        output.append("## God Nodes (most connected)")
        for i, node in enumerate(god_list[:5], 1):
            output.append(f"{i}. `{node['label']}` — {node['edges']} connections")

        output.append("")
        output.append("## Surprising Connections")
        if surprise_list:
            for s in surprise_list:
                output.append(f"- `{s['source_label']}` ↔ `{s['target_label']}` — {s.get('why', 'cross-community')}")
        else:
            output.append("- No cross-community connections detected")
        ...
        output.append("")
        output.append(f"## Output Files")
        output.append(f"- JSON: `{json_path}`")
        if html_path:
            output.append(f"- HTML (interactive): `{html_path}`")
        output.append(f"- Report: `{report_path}`")

        return "\n".join(output)

    except Exception as e:
        ctx.error(f"Error building knowledge graph: {str(e)}")
        return f"Error building knowledge graph: {str(e)}"


@mcp.tool(
    name="zotero_graph_god_nodes",
    description="Find the most connected items in your Zotero knowledge graph — core literature that bridges multiple topics."
)
def get_graph_god_nodes(
    top_n: int = 10,
    *,
    ctx: Context
) -> str:
    """Return the top_n most connected items from the knowledge graph."""
    try:
        from zotero_mcp.graph_builder import build_graph as _build_graph, TagFilter, graph_stats
        from zotero_mcp.graph_cluster import cluster, label_communities
        from zotero_mcp.graph_analyze import god_nodes

        # Try to load cached graph data
        json_path = _GRAPH_OUTPUT_DIR / "graph.json"
        if json_path.exists():
            ...
            return "No graph found. Run zotero_build_graph first."

        node_list = god_nodes(G, top_n=top_n)

        if not node_list:
            return "No connected items found in the graph."

        output = ["# God Nodes (Most Connected Items)", ""]
        ...
        # NetworkX version doesn't support edges="links"
        import networkx as nx
        from networkx.readwrite import json_graph as nx_json_graph
        with open(json_path) as f:
            data = json.load(f)
        G = nx_json_graph.node_link_graph(data)
        node_list = god_nodes(G, top_n=top_n)
        output = ["# God Nodes (Most Connected Items)", ""]
        for i, node in enumerate(node_list, 1):
            output.append(f"{i}. **{node['label']}** — {node['edges']} connections")
        ...
        ctx.error(f"Error finding god nodes: {str(e)}")
        return f"Error finding god nodes: {str(e)}"


@mcp.tool(
    name="zotero_graph_surprises",
    description="Find surprising cross-community connections in your Zotero knowledge graph — unexpected links between different research topics."
)
def get_graph_surprises(
    top_n: int = 5,
    *,
    ctx: Context
) -> str:
    """Return the most surprising cross-community connections."""
    try:
        from zotero_mcp.graph_cluster import cluster, label_communities, score_all
        from zotero_mcp.graph_analyze import surprising_connections
        ...
        # Reconstruct communities from JSON data
        communities = {}
        comm_data = data.get("communities", {})
        for cid_str, comm_info in comm_data.items():
            communities[int(cid_str)] = comm_info.get("members", [])

        community_labels = {
            int(cid_str): comm_info.get("label", f"Community {cid_str}")
            for cid_str, comm_info in comm_data.items()
        }

        surprise_list = surprising_connections(G, communities, top_n=top_n)

        if not surprise_list:
            return "No surprising cross-community connections found."

        output = ["# Surprising Connections (Cross-Community)", ""]
        for s in surprise_list:
            output.append(f"- **{s['source_label']}** ↔ **{s['target_label']}**")
            output.append(f"  {s.get('why', 'Cross-community connection')}")
            if s.get("shared_tags"):
                output.append(f"  Shared tags: {', '.join(s['shared_tags'])}")

        return "\n".join(output)

    except Exception as e:
        ctx.error(f"Error finding surprising connections: {str(e)}")
