...
def main():
    ...
    # Logging setup (stderr). Keep third-party noise down while allowing
    # zotero_mcp to be controlled by ZOTERO_LOG_LEVEL.
    ...
    zotero_level_name = os.getenv("ZOTERO_LOG_LEVEL", "INFO").upper()
    zotero_level = getattr(logging, zotero_level_name, logging.INFO)
    zotero_logger = logging.getLogger("zotero_mcp")
    zotero_logger.setLevel(zotero_level)
    zotero_logger.propagate = False
    if not zotero_logger.handlers:
        # Dedicated handler so debug isn't filtered by root handler levels
        zotero_handler = logging.StreamHandler(stream=sys.stderr)
        zotero_handler.setFormatter(logging.Formatter(log_format))
        zotero_handler.setLevel(zotero_level)
        zotero_logger.addHandler(zotero_handler)

    # Optional: silence specific loggers via env var, e.g.
    # ZOTERO_LOG_SILENCE=chromadb,httpx,urllib3
    silence = os.getenv("ZOTERO_LOG_SILENCE", "")
    if silence.strip():
        for name in [s.strip() for s in silence.replace(";", ",").split(",") if s.strip()]:
