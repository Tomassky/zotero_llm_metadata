...
# ---------------------------------------------------------------------------
# Synonym mapping (bilingual + Chinese near-synonyms)
# ---------------------------------------------------------------------------
...
def resolve_synonym(tag: str) -> str:
    ...
    If the tag is in SYNONYM_MAP, return the canonical form.
    Otherwise return the tag unchanged.
    """
...
def build_graph(
    ...
    *,
    ...
    for item in items:
        ...
        tags_normalized = [resolve_synonym(normalize_tag(t)) for t in tags_raw if t.strip()]
