...
- **Graph commands** (`graph-*`) are **offline** — they read `graph/graph.json`.
  **图谱命令**完全离线，读取 `graph/graph.json`。若提示 "No graph found"，先构建：
  ```
  python -m zotero_llm_metadata --build-graph      # run from the repo root / 在仓库根目录运行
  ```
...
- If graph results look stale vs what the user describes, offer to rebuild
  (`python -m zotero_llm_metadata --build-graph`) or fall back to live `search`.
...
- **`graph-*` says "No graph found"** → run `python -m zotero_llm_metadata --build-graph`.
  **图谱命令提示未找到图谱** → 运行 `python -m zotero_llm_metadata --build-graph`。
...
```
.claude/skills/zotero/          # installed copy / 已安装副本 (repo source: skill/)
  SKILL.md            # this file / 本文件
  bin/zot             # wrapper / 包装脚本
  requirements.txt    # runtime deps / 运行期依赖
  scripts/            # the CLI code / CLI 代码 (cli.py + modules, incl. synonyms.py)
  .venv/              # skill-local venv / 技能本地虚拟环境 (gitignored)
```

External deps: only the repo's `graph/graph.json`, built by
`python -m zotero_llm_metadata --build-graph`. Synonym normalization is fully
self-contained in `scripts/synonyms.py` (a vendored copy of the pipeline's
`graph/builder.py` map — no import of repo code). 外部依赖仅：仓库的
`graph/graph.json`；同义词归一已内置于 `scripts/synonyms.py`（流水线
`graph/builder.py` 映射表的副本，不再 import 仓库代码）。
